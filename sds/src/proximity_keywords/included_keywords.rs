...
impl<'a> IncludedKeywordSearch<'a> {
    ...
    pub fn next<'b>(&mut self, regex_caches: &'b mut RegexCaches) -> Option<usize> {
        let input = Input::new(self.content).range(self.start..).earliest(true);

        // TODO: use search_with with custom cache!
        if let Some(included_keyword_match) =
            self.keywords.keywords_pattern.content_regex.search_with(regex_caches.get(&self.keywords.keywords_pattern.content_regex), &input)
        {
...
#[cfg(test)]
mod test {
    use crate::proximity_keywords::{compile_keywords_proximity_config, CompiledIncludedProximityKeywords, IncludedKeywordSearch};
    ...
    fn test_included_keywords_on_start_boundary() {
        ...
        let keyword_matches = collect_keyword_matches(keywords
    ...
    fn should_detect_on_any_keyword() {
        ...
        assert_eq!(
            collect_keyword_matches(keywords
        ...
        assert!(collect_keyword_matches(keywords
