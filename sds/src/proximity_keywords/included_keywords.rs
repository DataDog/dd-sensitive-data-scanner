...
impl CompiledIncludedProximityKeywords {
    ...
    pub fn find_keyword_before_match(&self, match_start: usize, regex_caches: &mut RegexCaches, content: &str) -> Option<IncludedKeywordInfo> {
        ...
        while let Some(included_keyword_match) = keyword_search.next(regex_caches) {
            ...
            if is_index_within_prefix(content, included_keyword_match.start, match_start, self.look_ahead_character_count) {
                return Some(IncludedKeywordInfo {
                    keyword: content[included_keyword_match.start..included_keyword_match.end].to_string(),
