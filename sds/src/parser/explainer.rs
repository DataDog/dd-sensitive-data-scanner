...
mod tests {
    ...
    fn test_bracket_class_with_ranges() {
        ...
        for child in children {
            validate_all_positions(child, pattern);
        }
    }
    ...
    fn test_phone_number_like_pattern() {
        ...
        let tree = explain_regex(pattern).unwrap();
        validate_all_positions(&tree, pattern);
    }
    ...
    fn test_highlighting_safety() {
        ...
        for pattern in patterns {
            if let Ok(tree) = explain_regex(pattern) {
                test_all_positions_highlightable(&tree, pattern);
            }
        }
    }
    ...
    fn test_emoji_in_concatenation() {
        ...
        // Verify each part can be highlighted
        test_all_positions_highlightable(&tree, pattern);
    }
    ...
    fn validate_all_positions(node: &AstNode, pattern: &str) {
        assert!(
            node.start <= node.end,
            "Node {} has start {} > end {} (pattern: {})",
            node.node_type,
            node.start,
            node.end,
            pattern
        ...
        assert!(
            node.end <= pattern.len(),
            "Node {} has end {} > pattern len {} (pattern: {})",
            node.node_type,
            node.end,
            pattern.len(),
            pattern
    ...
    fn test_all_positions_highlightable(node: &AstNode, pattern: &str) {
        // Simulate UI highlighting - this should never panic
        if node.start < pattern.len() && node.end <= pattern.len() && node.start < node.end {
            let _ = &pattern[node.start..node.end];
        }

        if let Some(children) = &node.children {
            for child in children {
                test_all_positions_highlightable(child, pattern);
            }
        }
    }
}
    ...
    fn test_complex_pattern_with_escape_sequences() {
        // Test the Asana OAuth pattern with escape sequences in bracket classes
        // This pattern has: \\[rn] at the end which is an escaped backslash followed by [rn]
        let pattern = "\\b(?<sds_match>ey[I-L][\\w=-]+\\.ey[I-L][\\w=-]+\\.[\\w.+\\/=-]+)(?:$|[\\s)\\]}\"'>&]|\\\\[rn])";
        let result = explain_regex(pattern);
    ...
    fn validate_node_positions(node: &AstNode, pattern: &str) {
        assert!(node.start <= node.end, 
            ...
            node.start, node.end, node.node_type);
        assert!(node.end <= pattern.len(), 
            "End ({}) should be <= pattern length ({}) for {}", 
            node.end, pattern.len(), node.node_type);
        ...
        if node.start < pattern.len() && node.end <= pattern.len() && node.start < node.end {
            ...
            if node.node_type.contains("Group") {
                assert!(content.ends_with(')'), 
                    ...
                    node.node_type, node.start, node.end, content);
            }
        }
